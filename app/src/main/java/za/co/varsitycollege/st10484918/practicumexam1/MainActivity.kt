...
class MainActivity : AppCompatActivity() {
    ...
    Button1.setOnClickListener
    {
        ...
        //when statements//
        val string = when (nameofplaylist) {
            "The song title" -> "Millions"
            "The artist's name" -> "Young Thug"
            "The rating(1-5)" -> 4
            "User comments" -> "Rap"
            else -> "Please insert the given information"
        }
        val string = when (nameofplaylist) {
            "The song title" -> "Bad Blood"
            "The artist's name" -> "Taylor Swift"
            "The rating" -> 1
            "User comments" -> "Dance Song"
            else -> "Please insert the given information"
        }
        val string = when (nameofplaylist) {
